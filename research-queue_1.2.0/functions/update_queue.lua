...
function est_time(force)
	local est = {}
	local speed = 0
	for _, lab in pairs(global.labs[force.name]) do
		...
		speed = speed + ( (1 + speed_modifier) * (1 + productivity_modifier) )
	end
	speed = speed * (1 + force.laboratory_speed_modifier)
	for i, tech in ipairs(global.researchQ[force.name]) do
		if i == 1 then
			local t = 0
			if force.current_research then t = (1 - force.research_progress) * force.current_research.research_unit_count * force.current_research.research_unit_energy / speed end
			if force.current_research and force.current_research.name == tech then 
				est[i] = t
			else
			...
			if force.current_research and force.current_research.name == tech then 
				est[i] = (1 - force.research_progress) * force.current_research.research_unit_count * force.current_research.research_unit_energy / speed
			elseif i > 2 and force.current_research and global.researchQ[force.name][i-1] == force.current_research.name then
			...
			if not global.researchQ[force.name][1] then
				...
				for i, tech in ipairs(global.researchQ[force.name]) do
					...
					if global.offset_queue[index] < i and i <= (rq.q_per_page+global.offset_queue[index]) then
						...
						local caption = nil
						if time_estimation[i] < math.huge then caption = "Estimated time: "..util.formattime(time_estimation[i]) else caption = "Estimated time: Infinity" end

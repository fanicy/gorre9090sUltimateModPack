...
function trace_nixies()
  ...
  for k,v in pairs(global.nixie_tubes.nixies) do
    str=k.." = { "
...
local function onPlaceEntity(event)
  ...
  if entity.name=="nixie-tube" then
    ...
    if not global.nixie_tubes.nixies[entity.position.y] then
      global.nixie_tubes.nixies[entity.position.y]={}
    end
    ...
    local neighbor=global.nixie_tubes.nixies[pos.y][pos.x-1]
    if neighbor then
      debug("enslaving dude on the left")
      neighbor.slave = true
      while neighbor do
        setState(neighbor,"off")
        neighbor=global.nixie_tubes.nixies[neighbor.pos.y][neighbor.pos.x-1]
      end
    end
    --slave self to right, if any
    neighbor=global.nixie_tubes.nixies[pos.y][pos.x+1]
    if neighbor then
      debug("slaving to dude on the right")
      desc.slave=true
    end
    global.nixie_tubes.nixies[pos.y][pos.x] = desc
...
local function onRemoveEntity(entity)
  if entity.name=="nixie-tube" then
    local pos=entity.position
    local nixie_desc=global.nixie_tubes.nixies[pos.y] and global.nixie_tubes.nixies[pos.y][pos.x]
    if nixie_desc then
      removeSpriteObj(nixie_desc)
      global.nixie_tubes.nixies[pos.y][pos.x]=nil
      --if I had a slave, unslave him
      local slave=global.nixie_tubes.nixies[pos.y][pos.x-1]
      if slave then
        slave.slave=nil
        while slave do
          setState(slave,"off")
          slave=global.nixie_tubes.nixies[slave.pos.y][slave.pos.x-1]
        end
      end
    end
  end
          ...
          if not open and not desc.slave then
            ...
            if v and desc.has_power then
              ...
              repeat
                ...
                state = tostring(m)
                setState(d,state)
                d=global.nixie_tubes.nixies[d.pos.y][d.pos.x-1]
              until d==nil or v==0

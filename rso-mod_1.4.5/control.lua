...
--						end
	...
	local validCount, resOffsetX, resOffsetY
...
--			debug("Checking ".. newX .. "," .. newY .."("..location.x..","..location.y..")")
			local tile = surface.get_tile(newX,newY)
			if tile.valid and not tile.collides_with("water-tile") and surface.can_place_entity{name = rname, position = {x = newX,y = newY}} then
				location.valid = true
...
--				debug("Passed")
		...
		local ratio = validCount / #oreLocations
...
local function checkConfigForInvalidResources()
	...
	for resourceName, resourceConfig in pairs(config) do
		if game.entity_prototypes[resourceName] or resourceConfig.type == "entity" then
		...
		if resourceConfig.valid and resourceConfig.type ~= "entity" then
 			if game.entity_prototypes[resourceName].autoplace_specification == nil then
				resourceConfig.valid = false
